...
contract('PanToken',  function(accounts) {
  var tokenInstance;

it('initializes the contracts of the correct values', function(){
  return PanToken.deployed().then(function(instance) {
    ...
    assert.equal(standard, 'Pan Token v1.0', 'Has the correct standard');
  });
})
...
it('transfer token ownership', function(){
  return PanToken.deployed().then(function(instance) {
      tokenInstance =  instance;
      //Test 'require statement first by transferring comething larger than the sender's balacne
   ...
   assert.equal(receipt.logs[0].args._value, 250000, 'logs the transfer amount');
    ...
    assert.equal(balance.toNumber(), 250000, 'adds the amounts to the receiving accounts');
    return tokenInstance.balanceOf(accounts[0]);
  }).then(function(balance){
